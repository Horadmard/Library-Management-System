...
ostream & operator<<(ostream &x, Book a){
    x<<a.Code<<"-"<<a.Title<<endl<<"Authors: "<<a.Authors<<endl<<"Category: "<<a.Cat<<endl<<"Numbers: "<<a.Numbers<<endl;
    return x;
}
...
ostream & operator<<(ostream &o, User a){
    ...
    <<"Birthday: "<<a.BD<<endl<<"Father's name: "<<a.Fname<<endl<<"Password: "<<a.Pass;
    return o;
}
class Report{
    public:
...
ostream & operator<<(ostream &o, Report a){
    o<<"/"<<a.Start<<"/"<<endl<<"User("<<a.User_name<<")"<<endl<<"Book("<<a.Book_code<<")"<<endl<<"Expire("<<a.Expire<<")"<<endl
    <<"/"<<a.End<<"/"<<endl;
    return o;
}
//------------------------------------------
...
void _Reserve(User U,Book B,Date D,Report R,int Book_code,int User_name){
    ...
    R.Start=Today;
    R.End.Day=0;R.End.Month=0;R.End.Year=0;
    R.Expire=false;
...
void _Lend(User U,Book B,Date D,Report R){
    ...
    if(B.Numbers<=0 && B.Status==true){
        system("cls");
        cout<<"Not Available!"<<endl<<"Do you want to Reserve?\n1 - Yes\t0 - No\n";
        cin>>s;
        if(s){
            system("cls");
...
void Admin_PrintPopularBooks(Book B,Report R){
    ...
    for(int i=0;i<_FileSize(File_Name2)/sizeof(Book);i++){
        if(a[i]>m){
            ...
            cout<<B.Title<<"\t"<<B.Authors<<"\t"<<B.Cat<<endl<<"---------------"<<endl;
        }
    }
    Z.close();
    return;
...
void Admin_PrintActiveUsers(User U,Report R){
    ...
    for(int i=0;i<_FileSize(File_Name)/sizeof(User);i++){
        if(a[i]>m){
            ...
            X.read((char*)&U,sizeof(User));
            cout<<U.Name<<" "<<U.Family<<"\t"<<endl<<"---------------"<<endl;
        }
    }
    Z.close();
    X.close();
...
void _Return(User U,Book B,Date D,Report R){
    ...
    cout<<endl<<"Choose a Book to Return: ";
    cin>>c;
...
//------------------------------------------*/
    ...
    if(!Log_In(U))
        goto labble2;
    do{
        cout<<"Today: "<<Today<<endl<<"1 - Book List\n2 - Add new Book"<<endl;
    ...
    labble2:
    do{
        cout<<"Today: "<<Today<<endl<<"1 - Search a Book\n2 - Take a Book\n3 - My Returned Books"<<endl;
        cout<<"4 - My Books\n5 - Return a Book\n0 - Log_out\n"<<endl;
        cin>>Switch;
        switch(Switch){
            case 1:
